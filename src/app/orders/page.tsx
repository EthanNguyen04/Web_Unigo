...
export default function OrdersPage() {
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      if (dateRange[0] && dateRange[1]) {
        const startDate = moment.min(moment(dateRange[0]), moment(dateRange[1]));
        const endDate = moment.max(moment(dateRange[0]), moment(dateRange[1]));
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'total',
      key: 'total',
      render: (text) => `${text.toLocaleString('vi-VN')} VNĐ`, // Chỉ định locale 'vi-VN'
