...
import { Post_notification } from "../../config";
...
const Notifications: React.FC = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (!token) {
        throw new Error("Chưa đăng nhập, không thể gửi thông báo.");
      }
      ...
      // Lấy thời gian hiện tại làm thời gian hiển thị
      setNotifications(prev => [
        ...prev,
        { title: title.trim(), message: message.trim(), time: new Date() }
      ]);
      setShowDialog(false);
      setTitle("");
      setMessage("");
          ...
          <tbody>
            {notifications.map((n, idx) => (
              <tr key={idx} className="hover:bg-gray-50">
                ...
                <td className="p-2 border">{n.time.toLocaleString()}</td>
              </tr>
            ))}
