...
const CanceledOrders: React.FC = () => {
  ...
  const fetchCanceled = async () => {
    ...
    try {
      ...
      console.error("Lỗi khi tải đơn đã hủy:", error);
    } finally {
      setLoading(false);
    }
  };

  if (loading) return <div className="p-4 text-center">⏳ Đang tải đơn đã hủy…</div>;
  if (!orders.length) return <div className="p-4 text-center text-gray-500">Chưa có đơn nào đã hủy.</div>;
...
const OrderCard: React.FC<OrderCardProps> = ({ order }) => {
  ...
  return (
    <div className="bg-white rounded border shadow-sm p-4">
      <h3 className="font-bold mb-2 text-red-600">Mã đơn: {orderId}</h3>
